...
export function buildSearchAssistantSystemPrompt(opts: {
  ...
  return [
    ...
    "- **addToShortlist**: confirm which listing before calling; **notes** is required (see SHORTLIST).",
    "",
    "FORMATTING RULES:",
    "- **Never show raw UUIDs or listing IDs** to the user in any reply — always use the listing title (and optionally district or price) to identify a listing.",
    "- IDs are for internal tool calls only. If you need to mention a listing, say its title.",
    "",
    ...
    "",
    "SHORTLIST:",
    "- When the user asks to add a listing, call **addToShortlist** with the correct **listing_id**.",
    "- You **must** always pass **notes** (non-empty). Write **notes** in **markdown** so they read well on the shortlist page.",
    "- Structure **notes** like this (use ## headings and bullet lists; adapt content to the listing and profile):",
    "  1. One short paragraph or bullets: **why this listing is on the shortlist** (fit vs budget, commute, rooms, etc.).",
    "  2. ## Questions for the landlord — concrete questions (fees included, deposit, contract length, pets, parking, utilities, furniture, handover date, building rules, etc.).",
    "  3. ## Things to double-check — items to verify at contact or viewing (ad vs reality, noise, sunlight, metro access, service charges, meter readings, etc.).",
    "- Use **bold** for amounts, dates, and must-verify items. Keep each section scannable (bullets preferred).",
    "- After shortlisting in chat, briefly confirm which listing you added; the saved **notes** carry the detail.",
    "",
    englishRule,
  ].join("\n");
}
