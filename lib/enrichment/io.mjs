...
export async function fetchNormalizedListingsForEnrichment(
  supabase,
  {
    ...
    onlyActive = true,
    onlyMissing = true,
    missingAddressWithoutCoordsOnly = false,
  } = {},
) {
  ...
  if (missingAddressWithoutCoordsOnly) {
    if (availableColumns.has("address")) {
      query = query.is("address", null);
    }
    if (availableColumns.has("geocode_lat")) {
      query = query.is("geocode_lat", null);
    }
    if (availableColumns.has("geocode_lng")) {
      query = query.is("geocode_lng", null);
    }
  }
...
export function buildNormalizedEnrichmentUpdate({
  ...
  const payload = {
    ...
    geocode_provider: geocodeSucceeded
      ? enrichment.geocode.provider || "google_maps"
      : null,
    ...
    geocode_lat: geocodeSucceeded ? enrichment.geocode.latitude : null,
    geocode_lng: geocodeSucceeded ? enrichment.geocode.longitude : null,
...
export function buildListingEnrichmentRow({
  ...
  return {
    ...
    geocode_provider:
      geocode.status === "succeeded" ? geocode.provider || "google_maps" : null,
