...
import {
  applyDistrictFallbackScatter,
  buildGeocodeInput,
  buildDistrictFallbackGeocodeInput,
  describeDistrictFallbackRequirements,
  describeGeocodeQueryRequirements,
  DISTRICT_FALLBACK_QUERY_COMPONENTS,
...
test("district fallback requires district and city only", () => {
  const ready = buildDistrictFallbackGeocodeInput({
    streetHintRaw: null,
    districtBreadcrumbRaw: "Mokotow",
    searchCity: "Warszawa",
  });

  assert.equal(ready.status, "ready");
  assert.equal(ready.query, "Mokotow, Warszawa, Poland");

  const missing = describeDistrictFallbackRequirements({
    districtBreadcrumbRaw: null,
    districtHintRaw: null,
    district: null,
    searchCity: "Warszawa",
  ...
  assert.deepEqual(DISTRICT_FALLBACK_QUERY_COMPONENTS, ["district", "city"]);
  assert.equal(missing.isReady, false);
  assert.deepEqual(missing.missingComponents, ["district"]);
});
...
test("applyDistrictFallbackScatter adds deterministic slight jitter", () => {
  const listing = {
    normalizedListingId: "listing-42",
    source: "gratka",
    sourceListingId: "42",
  };
  const baseGeocode = {
  ...
  const first = applyDistrictFallbackScatter(baseGeocode, listing);
  const second = applyDistrictFallbackScatter(baseGeocode, listing);

  assert.equal(first.provider, "google_maps_district_fallback");
  assert.equal(first.partialMatch, true);
  assert.notEqual(first.latitude, 52.2);
  assert.notEqual(first.longitude, 21.0);
  assert.equal(first.latitude, second.latitude);
  assert.equal(first.longitude, second.longitude);
  assert.equal(first.payload.fallback_strategy, "district_centroid_scatter");
});

test("resolveRequestedCategories returns the fixed static proximity set", () => {
  const categories = resolveRequestedCategories();
  const keys = categories.map((category) => category.key);

  assert.deepEqual(keys, ["park", "gym", "grocery"]);
  assert.deepEqual(
    categories.find((category) => category.key === "grocery")?.includedTypes,
    [
      "grocery_store",
      "supermarket",
      "convenience_store",
  ...
  assert.deepEqual(
    categories.find((category) => category.key === "grocery")?.allowedBrandNames,
    ["Biedronka", "Lidl", "Żabka", "Zabka"],
  );
});
