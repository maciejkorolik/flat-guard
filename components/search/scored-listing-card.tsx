import type { NormalizedListing, ScoredListing } from "@/lib/types/flatguard";
import { MatchScore } from "./match-score";
import { MapPin, Calendar, Zap, CheckCircle, XCircle } from "lucide-react";
import { cn } from "@/lib/utils";

interface ScoredListingCardProps {
  listing: NormalizedListing;
  scored?: ScoredListing;
  animationDelay?: number;
}

const RECOMMENDATION_CONFIG = {
  strong: { label: "Strong Match", bg: "bg-[#e6faf7]", text: "text-[#006b5f]", border: "border-[rgba(0,107,95,0.2)]" },
  good: { label: "Good Match", bg: "bg-[#eff4ff]", text: "text-[#000666]", border: "border-[rgba(0,6,102,0.1)]" },
  weak: { label: "Weak Match", bg: "bg-[#fff7ed]", text: "text-[#b45309]", border: "border-[rgba(180,83,9,0.2)]" },
};

const CRITERION_COLORS: Record<string, string> = {
  budget: "bg-[#e6faf7] text-[#006b5f]",
  "budget fit": "bg-[#e6faf7] text-[#006b5f]",
  size: "bg-[#eff4ff] text-[#000666]",
  location: "bg-[#f0fdf4] text-[#15803d]",
  features: "bg-[#fef3c7] text-[#92400e]",
  availability: "bg-[#f5f3ff] text-[#6d28d9]",
};

function criterionColor(criterion: string): string {
  const key = criterion.toLowerCase();
  for (const [pattern, color] of Object.entries(CRITERION_COLORS)) {
    if (key.includes(pattern)) return color;
  }
  return "bg-[#f1f5f9] text-[#475569]";
}

export function ScoredListingCard({ listing, scored, animationDelay = 0 }: ScoredListingCardProps) {
  const isPending = !scored;
  const overallScore = scored?.overallScore;
  const breakdown = scored?.breakdown ?? [];
  const reasoning = scored?.reasoning;
  const recommendation = scored?.recommendation ?? "good";
  const rec = RECOMMENDATION_CONFIG[recommendation];

  return (
    <div
      className="bg-white border border-[rgba(198,197,212,0.15)] rounded-2xl shadow-sm overflow-hidden shrink-0 animate-fade-slide-in"
      style={{ animationDelay: `${animationDelay}ms`, animationFillMode: "both" }}
    >
      <div className="flex">
        {/* Left: listing info */}
        <div className="flex-1 p-6 flex flex-col gap-4 min-w-0">
          {/* Header row */}
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              <div className="flex items-center gap-2 mb-1">
                {isPending ? (
                  <span className="text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-full border bg-[#f1f5f9] text-[#94a3b8] border-[rgba(148,163,184,0.3)] flex items-center gap-1.5">
                    <span className="w-1 h-1 bg-[#94a3b8] rounded-full animate-pulse" />
                    Analysing…
                  </span>
                ) : (
                  <span
                    className={cn(
                      "text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-full border",
                      rec.bg,
                      rec.text,
                      rec.border
                    )}
                  >
                    {rec.label}
                  </span>
                )}
                {listing.offer_type && (
                  <span className="text-[10px] font-semibold text-[#767683] uppercase tracking-widest">
                    {listing.offer_type}
                  </span>
                )}
              </div>
              <h3 className="font-manrope font-bold text-[#0d1c2e] text-lg leading-tight line-clamp-1">
                {listing.title ?? "Apartment"}
              </h3>
              {listing.address && (
                <div className="flex items-center gap-1 text-[#767683] text-xs mt-1">
                  <MapPin size={10} />
                  <span className="line-clamp-1">{listing.address}</span>
                </div>
              )}
            </div>

            {/* Price */}
            <div className="text-right shrink-0">
              <div className="font-manrope font-extrabold text-[#000666] text-2xl whitespace-nowrap">
                {listing.total_monthly_pln
                  ? `${listing.total_monthly_pln.toLocaleString("pl-PL")} PLN`
                  : listing.rent_pln
                  ? `${listing.rent_pln.toLocaleString("pl-PL")} PLN`
                  : "Price TBD"}
              </div>
              <div className="text-[#767683] text-xs">/mo total</div>
            </div>
          </div>

          {/* Specs row */}
          <div className="flex items-center gap-4 flex-wrap">
            {listing.rooms != null && (
              <Spec label="Rooms" value={`${listing.rooms}`} />
            )}
            {listing.area_m2 != null && (
              <Spec label="Area" value={`${listing.area_m2} m²`} />
            )}
            {listing.floor != null && (
              <Spec label="Floor" value={`${listing.floor}`} />
            )}
            {listing.heating_type && (
              <Spec label="Heating" value={listing.heating_type} />
            )}
            {listing.is_furnished != null && (
              <div className="flex items-center gap-1">
                {listing.is_furnished ? (
                  <CheckCircle size={11} className="text-[#006b5f]" />
                ) : (
                  <XCircle size={11} className="text-[#94a3b8]" />
                )}
                <span className="text-xs font-medium text-[#454652]">
                  {listing.is_furnished ? "Furnished" : "Unfurnished"}
                </span>
              </div>
            )}
            {listing.has_balcony && (
              <div className="flex items-center gap-1">
                <CheckCircle size={11} className="text-[#006b5f]" />
                <span className="text-xs font-medium text-[#454652]">Balcony</span>
              </div>
            )}
            {listing.available_from && (
              <div className="flex items-center gap-1 text-[#767683] text-xs">
                <Calendar size={10} />
                <span>From {listing.available_from}</span>
              </div>
            )}
          </div>

          {/* Breakdown chips or skeleton */}
          {isPending ? (
            <div className="flex gap-2">
              {[80, 64, 72].map((w) => (
                <div
                  key={w}
                  className="h-6 rounded-full bg-[#f1f5f9] animate-pulse"
                  style={{ width: w }}
                />
              ))}
            </div>
          ) : breakdown.length > 0 ? (
            <div className="flex flex-wrap gap-2">
              {breakdown.map((item) => (
                <div
                  key={item.criterion}
                  className={cn(
                    "flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold",
                    criterionColor(item.criterion)
                  )}
                  title={item.note}
                >
                  <span>{item.criterion}</span>
                  <span className="font-bold">{item.score}/10</span>
                </div>
              ))}
            </div>
          ) : null}

          {/* AI reasoning */}
          {!isPending && reasoning && (
            <div className="flex items-start gap-2 bg-[rgba(0,6,102,0.03)] border border-[rgba(0,6,102,0.06)] rounded-xl px-4 py-3">
              <Zap size={11} className="text-[#000666] mt-0.5 shrink-0" />
              <p className="text-xs text-[#454652] leading-relaxed italic">
                &ldquo;{reasoning}&rdquo;
              </p>
            </div>
          )}
        </div>

        {/* Right: score panel */}
        <div className="w-36 bg-[rgba(239,244,255,0.4)] border-l border-[rgba(198,197,212,0.12)] flex flex-col items-center justify-center py-6 px-4 shrink-0 gap-4">
          {isPending || overallScore == null ? (
            <div className="flex flex-col items-center gap-2">
              <div className="w-20 h-20 rounded-full bg-[#f1f5f9] animate-pulse flex items-center justify-center">
                <span className="text-[#94a3b8] text-[10px] font-semibold">Scoring…</span>
              </div>
            </div>
          ) : (
            <MatchScore score={overallScore} />
          )}
          <button className="w-full bg-[#000666] text-white text-[11px] font-semibold py-2.5 rounded-xl hover:opacity-90 transition-opacity text-center">
            View Details
          </button>
        </div>
      </div>
    </div>
  );
}

function Spec({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-col">
      <span className="text-[10px] font-semibold uppercase tracking-widest text-[#767683] leading-none mb-0.5">
        {label}
      </span>
      <span className="text-sm font-semibold text-[#0d1c2e]">{value}</span>
    </div>
  );
}
