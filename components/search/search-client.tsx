...
function PhaseBar({ phase, found, scored, total }: {
  phase: Phase; found: number; scored: number; total: number;
}) {
  ...
  return (
    <div className="flex items-center gap-4">
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs font-semibold text-[#0d1c2e] whitespace-nowrap">Database scan</span>
        {!p1Done && <ScanningDots />}
        {p1Done && found > 0 && <span className="text-[11px] font-medium text-[#006b5f]">{found} found</span>}
      </div>
      ...
      <div className={cn("flex items-center gap-2", !p1Done && "opacity-30")}>
        <div className={cn(
          "w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0",
          p2Done ? "bg-[#8df5e4] text-[#006b5f]" : p2Active ? "bg-[#000666] text-white" : "bg-[rgba(0,6,102,0.1)] text-[#000666]"
        )}>
          {p2Done ? <CheckCircle2 size={12} /> : "2"}
        </div>
        <span className="text-xs font-semibold text-[#0d1c2e] whitespace-nowrap">AI scoring</span>
        {p2Active && <><ScanningDots /><span className="text-[11px] font-medium text-[#767683]">{scored} / {total}</span></>}
        {p2Done && <span className="text-[11px] font-medium text-[#006b5f]">complete</span>}
      </div>
    </div>
  );
}
...
export function SearchClient({ projectId, profile }: SearchClientProps) {
  ...
  const isReady = !!(profile?.preferred_cities?.length && profile?.budget_target_pln && profile?.rooms_preferred);
  ...
  const [modalListing, setModalListing] = useState<{ listing: NormalizedListing; scored?: ScoredListing } | null>(null);
  const started = useRef(false);

  // Load existing shortlist on mount
  useEffect(() => {
    fetch(`/api/shortlist/${projectId}`)
      .then((r) => r.json())
      .then(({ entries }) => {
        if (Array.isArray(entries)) {
          setShortlistedIds(new Set(entries.map((e: { listing_id: string }) => e.listing_id)));
        }
      })
      .catch(() => {});
  }, [projectId]);
  ...
  const scoredListings = useMemo(
    () => rawListings.map((l) => scoreMap.get(l.id)).filter(Boolean) as ScoredListing[],
    [rawListings, scoreMap]
  );
  ...
  const runSearch = useCallback(async () => {
    ...
    try {
      ...
      if (!listingsRes.ok) { setPhase("idle"); started.current = false; return; }
      ...
      if (!scoreRes.ok || !scoreRes.body) { setPhase("done"); return; }
  ...
  useEffect(() => {
    if (autorun && isReady && !started.current) runSearch();
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [autorun, isReady]);

  async function handleToggleShortlist(listing: NormalizedListing, scored?: ScoredListing) {
    const id = listing.id;
    if (shortlistedIds.has(id)) {
      // Remove
      setShortlistedIds((prev) => { const next = new Set(prev); next.delete(id); return next; });
      ...
      await fetch(`/api/shortlist/${projectId}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ listing_id: id, listing_snapshot: scored ?? { listing } }),
      });
    }
  }

  function handleShortlistFromChat(listingId: string) {
    setShortlistedIds((prev) => new Set(prev).add(listingId));
  }
  ...
  if (phase === "idle") {
    return <IdleView profile={profile} isReady={isReady} onRun={runSearch} />;
  }
  ...
  return (
    <>
      <div className="flex flex-col h-[calc(100vh-64px)]">
        {/* Header */}
        <div className="bg-[#eff4ff] border-b border-[rgba(198,197,212,0.12)] px-6 py-4 shrink-0">
          <div className="flex items-center justify-between gap-6">
            <div>
              <h2 className="font-manrope font-extrabold text-[#0d1c2e] text-lg tracking-tight mb-1.5">
                {isDone
                  ? `${rawListings.length} listings — AI scored`
                  : phase === "phase1"
                  ? "Scanning database…"
                  : `${rawListings.length} listings — scoring…`}
              </h2>
              <PhaseBar
                phase={phase}
            ...
            {isDone && (
              <div className="flex items-center gap-3 shrink-0">
                ...
                <button
                  onClick={handleRerun}
                  className="bg-[#000666] text-white flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-semibold hover:opacity-90 transition-opacity"
                >
          ...
          <div className="flex-1 min-h-0 min-w-0 overflow-y-auto px-6 py-5 space-y-4">
            {phase === "phase1" && (
              <div className="flex flex-col items-center justify-center py-20 gap-4">
                ...
                <p className="text-[#454652] text-sm font-medium">Querying database…</p>
              </div>
            )}
            {sorted.map((listing, i) => (
              <ScoredListingCard
                ...
                isShortlisted={shortlistedIds.has(listing.id)}
                animationDelay={i * 40}
                onOpenDetail={(l, s) => setModalListing({ listing: l, scored: s })}
      ...
      {modalListing && (
        <ListingDetailModal
          ...
          scored={modalListing.scored}
          isShortlisted={shortlistedIds.has(modalListing.listing.id)}
          onClose={() => setModalListing(null)}
          onToggleShortlist={(l, s) => { handleToggleShortlist(l, s); }}
        />
      )}
    </>
  );
...
function IdleView({ profile, isReady, onRun }: {
  profile: DbSearchProfile | null; isReady: boolean; onRun: () => void;
}) {
    ...
    <div className="flex flex-col items-center justify-center h-[calc(100vh-64px)] gap-8 bg-[#f8f9ff]">
      <div className="text-center max-w-md">
        ...
        <p className="text-[#767683] text-sm leading-relaxed">
          {isReady
            ? "Phase 1 pulls matching listings, then AI scores each one. Chat with the assistant to build your shortlist."
            : "Complete your interview first — we need at least a city, budget, and room count."}
        </p>
      </div>
      ...
      {profile && (
        <div className="flex flex-wrap gap-2 justify-center max-w-sm">
          ...
          {budget && <Chip label="Budget" value={`${budget.toLocaleString("pl-PL")} PLN`} color="primary" />}
      ...
      {isReady && (
        <p className="text-[#94a3b8] text-[11px]">Phase 1: database filter · Phase 2: AI scoring</p>
      )}
    </div>
  );
}

function Chip({ label, value, color }: { label: string; value: string; color: "primary" | "secondary" }) {
  return (
    <div className={cn(
